...
export class SocialService {
  ...
  async listPosts(params?: { type?: string; tag?: string; communityId?: string | null; q?: string }) {
    ...
    const media = await prisma.postMedia.findMany({ where: { postId: { in: ids } } })
    ...
    if (params && 'communityId' in params && params.communityId !== undefined) data = data.filter(p => p.communityId === params.communityId)
    if (params?.q) data = data.filter(p => (p.content || '').toLowerCase().includes((params.q as string).toLowerCase()))
